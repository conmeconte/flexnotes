...
const ToolbarButton = props =>
  ...
  >
    <span className="material-icons notesIcons">
      {props.icon}
    </span>
  </span>;
...
class Notes extends Component {
  ...
  componentWillMount() {
    ...
    if (tab_arr_obj) {
      ...
      if (
        tab_arr_obj[tabIndex].page_arr_obj[pageIndex].hasOwnProperty('notes')
      ) {
        const lastContent = JSON.parse(
          page_arr_obj[pageIndex].notes.document.content
        );
        this.setState({
  ...
  componentWillReceiveProps(nextProps, nextState) {
    if (nextProps.interface_obj.page_id !== this.props.interface_obj.page_id) {
      ...
      if (this.props.binderObj.tab_arr_obj) {
        ...
        for (let i = 0; i < tabArrLength; i++) {
          if (
            this.props.interface_obj.tab_id ===
            this.props.binderObj.tab_arr_obj[i]._id
          ) {
            if (
              this.props.interface_obj.tab_id ===
              nextProps.binderObj.tab_arr_obj[i]._id
            ) {
              tabIndex = i;
              break;
            }
        ...
        if (tabIndex !== null) {
          ...
          if (
            pageIndex !== null &&
            this.props.binderObj.tab_arr_obj[tabIndex].page_arr_obj[
              pageIndex
            ].hasOwnProperty('notes')
          ) {
  ...
  renderMarkButton = (type, icon) => {
    ...
    return (
      <span onMouseDown={onMouseDown} data-active={isActive} title={type}>
        <span className="material-icons notesIcons colorCircles richText">
          {icon}
        </span>
      </span>
    );
  ...
  onClickLink = event => {
    ...
    if (hasLinks) {
      ...
      change.insertText(text).extend(0 - text.length).call(wrapLink, href);
    }
  ...
  renderNode = props => {
    ...
    switch (node.type) {
      ...
      case 'bulleted-list':
        return (
          <ul className="notesUnorderedList" {...attributes}>
            {children}
          </ul>
        );
      case 'numbered-list':
      ...
      case 'link': {
        ...
        return (
          <a
            ...
            href={href}
            target="_blank"
            title="right-click on link to open"
          >
            {children}
          </a>
        );
    ...
    return (
      <div className="toolbar sixth-step">
        <div className="stylingButtons">
          ...
          >
            <span className="material-icons notesIcons">
              {this.state.isReadOnly ? 'lock' : 'lock_open'}
            </span>
          </span>
        </div>
        ...
        <div className="stylingButtons secondRow">
          <div className="font-dropdown">
            <span className="material-icons notesIcons richText">
              font_download
            </span>
            <div className="font-styles">
              <p className="fonts arial">
                {this.renderFontButton('arial', 'Arial')}
              </p>
              <p className="fonts comic">
                {this.renderFontButton('comic sans', 'Comic Sans')}
              </p>
              <p className="fonts courier">
                {this.renderFontButton('courier new', 'Courier New')}
              </p>
              <p className="fonts impact">
                {this.renderFontButton('impact', 'Impact')}
              </p>
              <p className="fonts roboto">
                {this.renderFontButton('roboto', 'Roboto')}
              </p>
              <p className="fonts times">
                {this.renderFontButton('times new roman', 'Times New Roman')}
              </p>
            </div>
          </div>

          <div className="hoverOptions">
            <span className="hoverDropbtn">
              <i className="material-icons fontColorIcon notesIcons">
                format_color_text
              </i>
            </span>
            <div className="fontColor-options">
              <p className="fontColor redFont">
                {this.renderMarkButton('red', 'lens')}
              </p>
              <p className="fontColor orangeFont">
                {this.renderMarkButton('orange', 'lens')}
              </p>
              <p className="fontColor yellowFont">
                {this.renderMarkButton('yellow', 'lens')}
              </p>
              <p className="fontColor greenFont">
                {this.renderMarkButton('green', 'lens')}
              </p>
              <p className="fontColor blueFont">
                {this.renderMarkButton('blue', 'lens')}
              </p>
              <p className="fontColor violetFont">
                {this.renderMarkButton('purple', 'lens')}
              </p>
            </div>
          </div>
          ...
          <div className="hoverOptions">
            <span className="hoverDropbtn">
              <i className="material-icons emojiIcon notesIcons">
                insert_emoticon
              </i>
            </span>
                ...
                {EMOJIS.map((emoji, i) => {
                  ...
                  return (
                    <span key={i} className="button" onMouseDown={onMouseDown}>
                      <span className="material-icons textAreaEmoji">
                        {emoji}
                      </span>
                    </span>
                  );
        ...
        <h6 className="saveNotes">
          {this.state.save ? 'Notes saved' : ''}
        </h6>
      </div>
    );
  };
  ...
  render() {
    return (
      <div className="text-editor">
        <div className="notes-component-toolbar">
          {this.toolbar()}
        </div>
