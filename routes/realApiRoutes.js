...
module.exports = app => {
  ...
  app
    ...
    .post('/api/tab', requireLogin, async (req, res) => {
      ...
      const existingUser = await User.findById(req.user.id, function(
        err,
        user
      ) {
        if (err) {
      ...
      if (existingUser) {
        ...
        if (binder) {
          ...
          res.send('Binder Array does not exist');
        }
      } else {
        res.send("Error can't find user");
      }
    ...
    .delete('/api/tab', requireLogin, async (req, res) => {
      ...
      if (existingUser) {
        const tab = existingUser.binder_arr_obj
  ...
  app
    ...
    .post('/api/page', requireLogin, async (req, res) => {
      ...
      if (existingUser) {
        ...
        if (tab) {
          ...
          res.send('Binder/tab id does not exist');
        }
      } else {
        res.send("Error can't find user");
      }
    ...
    .put('/api/page', requireLogin, async (req, res) => {
      ...
      if (existingUser) {
        ...
        page.panel_dimensions.top_left_panel_height =
          req.body.top_left_panel_height ||
          page.panel_dimensions.top_left_panel_height;
        page.panel_dimensions.top_left_panel_width =
          req.body.top_left_panel_width ||
          page.panel_dimensions.top_left_panel_width;
        page.panel_dimensions.top_right_panel_height =
          req.body.top_right_panel_height ||
          page.panel_dimensions.top_right_panel_height;
        page.panel_dimensions.number_of_panels =
          req.body.number_of_panels || page.panel_dimensions.number_of_panels;
